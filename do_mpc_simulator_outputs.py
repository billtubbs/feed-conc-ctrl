...
for k in range(5):
    ...
    print(
        f"  k={k}: t={t_k:.1f}, x[k]={x_k:.2f}, y[k]={y_k:.2f}, Match={match}"
    )
...
for k in range(4):
    ...
    print(
        f"  k={k}: t={t_k:.1f}, x[k+1]={x_k_plus_1:.2f}, y[k]={y_k:.2f}, Match={match}"
    )
...
if abs(sim_results.loc[0, ("x", 0)] - sim_results.loc[0, ("y", 0)]) < 0.01:
    ...
    print(
        "✓ y[k] = x[k+1]: Output computed from NEXT state (after integration)"
    )
    print("\nThis means at row k:")
    print("  - t = k")
