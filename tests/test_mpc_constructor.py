...
def two_tank_mixer_system():
    ...
    systems = [
        MixingTankModelCT(D=D, name=name) for name in tank_names
    ]
...
def bounds():
    ...
    return {
        ...
        'disturbances': {
            'tank_1_conc_in': {'lower': 0.0, 'upper': 10.0},
            'tank_2_conc_in': {'lower': 0.0, 'upper': 10.0},
            'mixer_v_dot_out': {'lower': 0.0, 'upper': 5.0},
        },
        'outputs': {
...
def test_construct_mpc_creates_controller(
    ...
    for mv_name, mv_bounds in bounds['inputs'].items():
        if 'lower' in mv_bounds:
            assert mpc.bounds['lower', '_u', mv_name] == (
                mv_bounds['lower']
            )
        if 'upper' in mv_bounds:
            assert mpc.bounds['upper', '_u', mv_name] == (
                mv_bounds['upper']
            )

    # Check bounds on states
    for state_name, state_bounds in bounds['states'].items():
        if 'lower' in state_bounds:
            assert mpc.bounds['lower', '_x', state_name] == (
                state_bounds['lower']
            )
        if 'upper' in state_bounds:
            assert mpc.bounds['upper', '_x', state_name] == (
                state_bounds['upper']
            )

    # Check bounds on disturbances
    for dist_name, dist_bounds in bounds['disturbances'].items():
        if 'lower' in dist_bounds:
            assert mpc.bounds['lower', '_x', dist_name] == (
                dist_bounds['lower']
            )
        if 'upper' in dist_bounds:
            assert mpc.bounds['upper', '_x', dist_name] == (
                dist_bounds['upper']
            )
...
def test_mpc_simulation_matches_notebook(
    ...
    notebook_results = pd.read_csv(
        test_data_dir / "feed_conc_ctrl_sim_results.csv",
        header=[0, 1]
    )
    ...
    for mv_name in control_design['manipulated_variables']:
        ...
        notebook_values = (
            notebook_results['manipulated_inputs'][mv_name].values
        )
